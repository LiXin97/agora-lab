...
<p align="center">
  <img src="docs/assets/readme/runtime-dashboard.png" alt="运行中的仪表板：看板、决策日志和系统健康（L2 解死锁后）" width="90%" />
</p>
<p align="center">
  <img src="docs/assets/readme/lab-view.png" alt="Lab View：像素风监控视图" width="90%" />
</p>
...
- **运行时自动化（L1+L2）**：常驻 watchdog 主动把闲置 agent 拉回工作，无需人工轮询
  - 签名差分注入：当某个 agent 的收件箱／任务／会议状态变化时，watchdog 立刻向其 tmux 面板注入定制 prompt
  - L1 闲置心跳：曾被注入过、但超过心跳阈值（默认 20 分钟）仍无信号变化的 agent 会被重新触发会话启动检查表 — 打破"无事件 ⇒ 无注入 ⇒ 永久死锁"
  - L2 supervisor 编排：每个周期聚合全局视图（in_progress 卡死 >2h、Review 列空但仍有 in_progress、会议阶段停滞、`#ID` 字符串依赖链）并向 supervisor 叠加一个带行动策略的 prompt（执行根本阻塞项、通过 `agora kanban` 重新分配、或写一份 `supervisor_to_user_*_status.md`，禁止静默 idle）。30 分钟桶去重避免噪声
  - Pane-busy 守卫：检测到 Claude Code TUI 正在推理时跳过注入，避免 prompt 被堆叠成无效 paste 块
...
**会议是手动触发的。**没有自动周期——只有当积累的材料足够支撑对抗式讨论时，导师才会显式开会（命令行 `agora meeting new`，或 Dashboard 里点 **Start Review Meeting**）。会议之间，整个实验室靠 kanban 任务和 `shared/messages/` 里的文件消息事件驱动运行。`lab.yaml` 中记录 `meeting.trigger: manual`、`min_participants`、`decision_maker`；每次会议的参与者从学生和 research staff 中挑选。Paper Reviewer 永远不会被选中——服务端会拒绝包含 reviewer 的会议，Dashboard 的参与者选择器也会把 reviewer 过滤掉。

只有相应产物到位，阶段才会前进。`PREPARE` 要求每位参与者写完 perspective（research staff 还需写完 judgment）；`CROSS_READ` 要求每位参与者提交 judgment；`CHALLENGE` 至少需要一条 critique；`RESPOND` 要求除决策者外的每位参与者完成回应；`DECISION` 为终态。

## Paper Review 工作流

Paper Reviewer 位于常规实验室循环的**外部**：不参加组会、不跟学生/导师交叉讨论、不从通用 kanban 里主动捡任务。他们只处理导师显式派发给他们的 review 任务。

### 任务派发格式（导师侧）

每个 reviewer 任务的标题/描述必须包含两项——论文指针 **和** 目标会议：
...
缺任一字段（指针或目标会议）的任务都不可用——在 reviewer 接单之前先把描述补完整。

### Reviewer 执行流程

每个 reviewer 对每个任务执行一个固定三步流水线：

1. **定位论文** —— 按给定的 arXiv id / pdf 抓取，或在给定的时间窗内按 topic 搜 arXiv、挑最相关的一篇。
2. **识别目标会议** —— 查阅该会议的官方审稿说明，**原样复刻**其 review 表单（章节标题、必填字段、评分量表、confidence 量表、checklist）。不跨会议拼接或臆造字段。
3. **撰写 review** —— 把每个必填字段写完整。输出到 `shared/paper-reviews/<paperId>/rounds/R1/reviews/<reviewer-name>.md`，文件头 YAML frontmatter 必须包含 `paper_arxiv_id`、`paper_title`、`target_conference`、`reviewer`、`completed_at`。

每轮的导师汇总仍然写到该 case 目录下的 `supervisor-resolution.md`。反复迭代直到草稿达到投稿标准。
